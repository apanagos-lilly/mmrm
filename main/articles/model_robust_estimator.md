...
the G-computation estimator in the context of the MMRM and describe an
...
The covariance matrix of \\\hat{\theta}\_t\\ can be written in the
following format: \\ Var(\hat{\theta}\_t) = E\[
Var(\hat{\theta}\_t~\|~X) \] + Var\[E(\hat{\theta}\_t~\|~ X)\] \\

Many variance estimators focus on the first term. To enable a more
general correct variance estimator that is robust to model
misspecification, we allow `mmrm` to add back the second term, which is
related to the randomness of baseline covariates used in the design
matrix.

To account for the extra source of variation from treating covariates as
random, define the vector of subject-level contributions to the
G-computation estimator at time \\t\\, \\ \hat{\mathbf{v}}\_i =
...
sample covariance \\ \hat\Sigma_v = \frac{1}{n-1}\sum\_{i=1}^n
...
from the model while treating baseline variables as fixed, the corrected
covariance matrix is \\ \hat\Omega^c = \hat{\Omega}\_{\theta} +
\frac{1}{n} \hat{\Sigma}\_v. \\

## Implementation

The G-computation estimator and correct variance can be implemented
using the `emmeans` package. To enable correct variance estimation,
